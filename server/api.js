...
async function insertOne(username, profilePic, bestScore, dbName, collectionName) {
  const client = new MongoClient(url, { useNewUrlParser: true, useUnifiedTopology: true });
  ...
  try {
    ...
    var obj = {username: username, profilePic: profilePic, bestScore: bestScore};

    const result = await collection.insertOne(obj);
    console.log(`데이터가 성공적으로 삽입되었습니다. ID: ${result.insertedId}`);
  } catch (error) {
    console.error('MongoDB 연결 중 오류 발생:', error);
  } finally {
    await client.close();
...
async function deleteOneByName(target, dbName, collectionName) {
  ...
  try {
    ...
    if (result.deletedCount > 0) {
      console.log(`삭제 완료: ${result.deletedCount}개의 문서가 삭제되었습니다.`);
    } else {
